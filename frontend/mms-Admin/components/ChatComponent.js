...
function ChatComponent({ receiverId, isModelChatClose }) {
  ...
  useEffect(() => {
    ...
    channel.bind(channelName, (data) => {
      ...
      setMessages((prevState) => [...prevState, data?.chat]);
    });
  }, [receiverId]);
        ...
        <div className={styles.chatBox}>
          ...
          <div className={styles.chatBox_main} ref={containerRef}>
            {messages.length > 0 ? (
                ...
                <>
                  <div
                    ...
                    className={`${styles.message} ${
                      msg?.sender_id === user?.id ? styles.sent : ""
                    }`}>
                    <div>{msg?.message}</div>
              ...
              <>
                <div className={styles.message}>Select a user to chat with</div>
              </>
            )}
            <div ref={chatBoxRef} />
          </div>
